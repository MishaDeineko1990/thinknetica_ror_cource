...
# Ruby Blocks: www.rubyguides.com             https://www.rubyguides.com/2016/02/ruby-procs-and-lambdas/

File.open("1.txt").each { |line| puts line }
якщо після відкриття файлу відкрити блок то не прийдеться закривати файл 

a = Proc.new {}
b = proc. {}
# lambda контролює кількість переданих аргументів
c = lambda {}
->(x) { puts x }

# ------------------------------------
#  Бдоки в рубі є замкненями
#     - замикання цеце фунцкії які зберігають стан оточення, стан локальних змінних в яких вони були визначені
# ------------------------------------    
...
def m2 (str, &block)  # &block для перевірки
  if block_given?     # перевірка на передання функції
    yield(str)        # виклик функції 
  else 
    puts str
  end  
end

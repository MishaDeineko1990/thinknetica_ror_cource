...
class Interface
    ...
    @stations.each_with_index do |station, i|
      ...
      station.each_block do |train, ind|
        ...
        train.each_block do |wagon, index|
          puts "#{index + 1}. #{wagon.type}, full volume#{wagon.full_volume},
          reserved volume: #{wagon.filled}, free volume: #{wagon.available_volume}"
        end
      end
    end
  end
  ...
  def t_all
    puts "***#{__method__}****"
    puts Train.trains.inspect.to_s
  end
  ...
  def show_detal_info_train(train = 0)
    ...
    puts "Station position: #{train.station_position.name}" unless train.station_position.nil?
  ...
  def add_wagon
    puts "***#{__method__}****"
    number_train = choose_train
    case @trains[number_train].type
    when 'cargo'
      puts 'Write value wagon'
      @trains[number_train].add_wagon(CargoWagon.new(gets.chomp.to_i))
    when 'passenger'
      puts 'Write count sits in wagon'
      @trains[number_train].add_wagon(PassengerWagon.new(gets.chomp.to_i))
    end
  ...
  def show_list_routes(routes = @routes)
    ...
    routes.each_with_index do |value, index|
      print "#{index + 1}) "
      value.list.each_with_index do |value2, ind|
        print "#{ind + 1}. #{value2.name}, " if value2 != value.list.last
        print "#{ind + 1}. #{value2.name}\n" if value2 == value.list.last
      end
    end
    equal_line
  end
